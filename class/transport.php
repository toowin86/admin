...
class Transport{
	...
	function __construct(){
	   if (isset($_SESSION['a_options']['smsintel: login']) 
       and isset($_SESSION['a_options']['smsintel: password'])
       and $_SESSION['a_options']['smsintel: login']!=''
       and $_SESSION['a_options']['smsintel: password']!=''
       ){
    	define("HTTPS_LOGIN", $_SESSION['a_options']['smsintel: login']); //Ваш логин для HTTPS-протокола
    	define("HTTPS_PASSWORD", $_SESSION['a_options']['smsintel: password']); //Ваш пароль для HTTPS-протокола
        ...
        define("USE_HTTPS", 0); //1 - использовать HTTPS-адрес, 0 - HTTP
            ...
            echo 'Не указаны настройки smsintel: a_options->smsintel: login = "'.$_SESSION['a_options']['smsintel: login'].'" и a_options->smsintel: password = "'.$_SESSION['a_options']['smsintel: password'].'"';
            exit;
        }
	}
}
