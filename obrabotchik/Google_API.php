...
//Подключаем библиотеку Google API
 ...
 $inc_=_GP('inc_','?inc=c_call_client');
 ...
 if ($err_google==''){
     $client->setClientId($_SESSION['a_options']['Google Client Secret - для Google Drive']);//$_SESSION['a_options']['Google Client Secret - для Google Drive']
     $client->setClientSecret($_SESSION['a_options']['Google Redirect Uri - для Google Drive']);//$_SESSION['a_options']['Google Redirect Uri - для Google Drive']
     $client->setRedirectUri('http://'.$_SERVER['SERVER_NAME'].'/admin/'.$inc_);
     ...
     if (isset($_REQUEST['code']) || (isset($_SESSION['google_drive_access_token']) && $_SESSION['google_drive_access_token'])) {
        if (isset($_REQUEST['code'])) {
            ...
            header('Location: http://'.$_SERVER['SERVER_NAME'].'/admin/'.$inc_);
        } else{
