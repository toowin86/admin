...
(function ($, window, document) {
    ...
    PeriodPicker.prototype.applyOptions = function () {
        ...
        if (options.withoutBottomPanel || (!options.clearButton && (!options.okButton || options.inline) && !options.showDatepickerInputs && (!options.showTimepickerInputs || !options.timepicker || $.fn.TimePicker === undefined))) {
    ...
    PeriodPicker.prototype.show = function (target) {
        var i, offset, top, left, self = this;
        if (!self.options.inline) {
            ...
            if (self.options.fullsize) {
                ...
                offset = !self.options.likeXDSoftDateTimePicker ? self.button.offset() : $(target).offset();

                top = offset.top + (!self.options.likeXDSoftDateTimePicker ? self.button.outerHeight() : $(target).outerHeight()) - 1;
                left = offset.left;

                if (top + self.picker.outerHeight() > $(window).height() + $(window).scrollTop()) {
                    top = offset.top - self.picker.outerHeight() - 1;
                }
                if (top < 0) {
                    top = 0;
                }
                if (left + self.picker.outerWidth() > $(window).width()) {
                    left = $(window).width() - self.picker.outerWidth();
                }
