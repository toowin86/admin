<?php 
     ...
     // добавить товары в заказ, через запятую, : - количество ?inc=m_zakaz&add_id=33447:2,33448,33446
?>
    	        ...
    	        if (is_json(data)==true){
    	            ...
    	            if (typeof data_n['i']=='object'){
    	               for(var i in data_n['i']){
                            ...
                            s_cat_add(s_cat_arr['id'],s_cat_arr['name'],cnt_,s_cat_arr['price'],s_cat_arr['tip'],'',barcode,s_cat_arr['img'],s_cat_arr['pr']);
    ...
    if (th_.find('.s_cat_cur_barcode').size()>0){
        ...
        if (barcode!=''){
            ...
            for(var i in barcode_arr){
                ...
                //alert(barcode_arr[i]+' - '+arr[3]);
                if (arr[2]>0){//если товар в наличии
                
                    var disabled_='disabled="disabled"';var sel_='';
                    if (arr[3]>0){
                        //alert('+'+sel_);
                        sel_=' class="selected"';disabled_='';
                     }
                    
                    var kol_txt='';if ((arr[2]-0)>1){kol_txt='<input type="text" class="part_kol" placeholder="Кол." data-max="'+_IN(arr[2])+'" value="'+arr[3]+'" '+disabled_+' />';}
                    val+='<li'+sel_+' value="'+arr[0]+'" data-barcode="'+_IN(arr[1])+'" data-kol="'+_IN(arr[2])+'">'+arr[1]+' '+kol_txt+'</li>';
                }
            }
        }
    }
